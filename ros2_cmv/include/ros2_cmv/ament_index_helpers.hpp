...
inline std::vector<Interface> listInterfaces()
{
    ...
    for (const auto &package : packages)
    {
        ...
        while (std::getline(iss, line))
        {
            ...
            if (line.find(".msg") != std::string::npos)
            {
                ...
                knownInterface.package = package_name;
                knownInterface.base_name = base_name;
                known_interfaces.push_back(knownInterface);
            }
        }
    }
...
inline std::string getMsgPath(std::string& package_name, std::string& msg_name)
{
    ...
    // if (!msg_file.is_open())
    // {
    //     std::cerr << "Failed to open message file at: " << msg_file_path << "\n";
    //     return;
    // }

    // std::cout << "Contents of " << package_name << "/msg/" << msg_name << ".msg:\n";
    // std::string line;
